...
volatile unsigned int ligne=0, colonne=0;
...
/* Configure the LCD Control pins --------------------------------------------*/
  ...
  temp = GPIOD->CRL & ~0x00FF00FF;                          /* clear Bits */
  GPIOD->CRL = temp |  0x00BB00BB;                          /* alternate function output Push-pull 50MHz */
  /* PD.08(D13), PD.09(D14), PD.10(D15), PD.14(D0), PD.15(D1) */
  temp = GPIOD->CRH & ~0xFF000FFF;                          /* clear Bits */
  GPIOD->CRH = temp |  0xBB000BBB;                          /* alternate function output Push-pull 50MHz */
   
  /* PE.07(D4) */ 
  temp = GPIOE->CRL & ~0xF0000000;                          /* clear Bits */
  GPIOE->CRL  = temp | 0xB0000000;                          /* alternate function output Push-pull 50MHz */
  /* PE.08(D5), PE.09(D6),  PE.10(D7), PE.11(D8), PE.12(D9), PE.13(D10), PE.14(D11), PE.15(D12) */
  temp = GPIOE->CRH & ~0xFFFFFFFF;                          /* clear Bits */
  GPIOE->CRH  = temp | 0xBBBBBBBB;                          /* alternate function output Push-pull 50MHz */

  /* PF.00(A0 (RS)) */ 
  temp = GPIOF->CRL & ~0x0000000F;                          /* clear Bits */
  GPIOF->CRL  = temp | 0x0000000B;                          /* alternate function output Push-pull 50MHz */

  /* PG.12(NE4 (LCD/CS)) - CE3(LCD /CS) */
  temp = GPIOG->CRH & ~0x000F0000;                          /* clear Bits */
  GPIOG->CRH  = temp | 0x000B0000;                          /* alternate function output Push-pull 50MHz */

  /* PA.08(LCD Backlight */
  GPIOA->BRR |=  0x00000100;                          /* Backlight off */
  temp = GPIOA->CRH & ~0x0000000F;                          /* clear Bits */
  GPIOA->CRH  = temp | 0x00000003;                          /*                    output Push-pull 50MHz */
...
void GLCD_AfficheString (unsigned int ln, unsigned int col, unsigned char fi, unsigned char *s) {
  GLCD_WindowMax();
  while (*s) {
    GLCD_DisplayChar(ln, col, fi, *s);
    s++;
    col++;
    if ( col >= NB_COLONNE ) {  // 14 colonne max en mode vertical si Font
        col=0;
        ln++;
        if ( ln >= NB_LIGNE )
            ln=0;
...
*                   //val:      value of active bargraph (in pixels) � condition de commenter la ligne  val = (val * w) >> 10;           *
*   Return:                                                                    *
*******************************************************************************/
...
void GLCD_gotoXY (unsigned int ln,unsigned int col){
    ligne=ln; colonne=col;
} 
